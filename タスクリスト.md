...
### 6.4 GitHub Pages設定
- [x] GitHubリポジトリの作成
- [x] `.gitignore`ファイルの作成（.NET用）
- [x] Blazor WebAssemblyのベースパス設定
  - `index.html`の`<base href="/" />`を`<base href="/リポジトリ名/" />`に変更
- [x] 404.htmlの作成（SPA用リダイレクト対応）

### 6.5 GitHub Actions設定
- [x] `.github/workflows`フォルダの作成
- [x] `deploy.yml`ワークフローファイルの作成
  - .NET SDKのセットアップ
  - 依存関係のリストア
